...
export function registerEVMTools(server: McpServer) {
  ...
  // Our new embedding getting token transfers data
server.tool(
  "get_token_transfers",
  "Get ERC20 token transfer details for a specific token address with optional date filtering",
  {
    tokenAddress: z.string().describe("The ERC20 token contract address"),
    fromDate: z.string().optional().describe("Start date for filtering transfers in YYYY-MM-DD format (e.g., '2024-01-01'). If not provided, gets all available transfers."),
    toDate: z.string().optional().describe("End date for filtering transfers in YYYY-MM-DD format (e.g., '2024-12-31'). If not provided, gets up to current date.")
  },
  async ({ tokenAddress, fromDate, toDate }) => {
    try {
      const transfers = await services.getTokenTransfers(tokenAddress, fromDate, toDate);
      return {
        content: [{
          type: "text",
          text: services.helpers.formatJson(transfers)
        }]
      };
    } catch (error) {
      return {
        content: [{
          type: "text",
          text: `Error fetching token transfers: ${error instanceof Error ? error.message : String(error)}`
        }],
        isError: true
      };
    }
...
// new em,bedding for nft transfer history
server.tool(
  "get_nft_transfers",
  "Get ERC721 NFT transfer details for a specific contract address with optional token ID and date filtering",
  {
    contractAddress: z.string().describe("The ERC721 NFT contract address"),
    tokenId: z.string().optional().describe("Specific NFT token ID to filter transfers for (e.g., '390'). If not provided, gets transfers for all tokens in the collection."),
    fromDate: z.string().optional().describe("Start date for filtering transfers in YYYY-MM-DD format (e.g., '2024-01-01'). If not provided, gets all available transfers."),
    toDate: z.string().optional().describe("End date for filtering transfers in YYYY-MM-DD format (e.g., '2024-12-31'). If not provided, gets up to current date.")
  },
  async ({ contractAddress, tokenId, fromDate, toDate }) => {
    try {
      const transfers = await services.getNFTTransfers(contractAddress, tokenId, fromDate, toDate);
      return {
        content: [{
          type: "text",
          text: services.helpers.formatJson(transfers)
      ...
      return {
        content: [{
          type: "text",
          text: `Error fetching NFT transfers: ${error instanceof Error ? error.message : String(error)}`
        }],
        isError: true
      };
    }
